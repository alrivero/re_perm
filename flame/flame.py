...
from .mesh import face_vertices
from .lbs import lbs, vertices2landmarks, blend_shapes, vertices2joints, batch_rodrigues
...
# FLAME_MODEL_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/generic_model.pkl"
FLAME_MODEL_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/flame2023.pkl"
FLAME_MESH_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/head_template_mesh.obj"
FLAME_PARTS_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/FLAME_masks.pkl"
FLAME_LMK_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/landmark_embedding_with_eyes.npy"
FLAME_TEX_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/FLAME_texture.npz"
FLAME_PAINTED_TEX_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/tex_mean_painted.png"
FLAME_UVMASK_PATH = "/home/alrivero/ENPC/re_perm/flame/assets/uv_masks.npz"
...
class FlameHead(nn.Module):
    ...
    def forward(
        ...
        *,                         # everything after this is keyword-only
        ...
        Returned tuple order (items omitted when their flag is False):

            vertices   : (B, V, 3)  float32
            v_cano     : (B, V, 3)
            landmarks  : (B, L, 3)
            faces      : (F, 3)     int64
            normals    : (B, F, 3)  (F,3) when B==1
            mesh_tm    : trimesh.Trimesh  or None
        """
        device, B = shape.device, shape.shape[0]
        ...
        v_shaped   = v_template + blend_shapes(betas, self.shapedirs)

        if dynamic_offset is not None:
            v_shaped = v_shaped + dynamic_offset

        vertices, J, _ = lbs(
            full_pose, v_shaped, self.posedirs,
        ...
        if return_faces or return_normals or return_trimesh:
            faces_torch = self.faces.to(device)  # (F,3) int64

        if return_normals or return_trimesh:
            v0 = vertices[:, faces_torch[:, 0], :]
            v1 = vertices[:, faces_torch[:, 1], :]
            v2 = vertices[:, faces_torch[:, 2], :]
            normals = torch.cross(v1 - v0, v2 - v0, dim=-1)
            normals = normals / (normals.norm(dim=-1, keepdim=True) + 1e-8)
            if B == 1:
                normals = normals.squeeze(0)      # (F,3) for batch=1
        ...
        if return_trimesh:
            try:
                ...
                faces_np  = faces_torch.detach().cpu().numpy()
                norms_np  = normals.detach().cpu().numpy()
                mesh_tm   = trimesh.Trimesh(vertices=verts_np,
                                            faces=faces_np,
                                            face_normals=norms_np,
                                            process=False) 
            except Exception:
                mesh_tm = None  # silently ignore if trimesh unavailable
        ...
        if return_landmarks:
            lmk = vertices2landmarks(
                vertices,
                self.faces,
                self.full_lmk_faces_idx.repeat(B, 1),
                self.full_lmk_bary_coords.repeat(B, 1, 1),
            )
            ret.append(lmk)

        if return_faces:
            ret.append(faces_torch[None])

        if return_normals:
            ret.append(normals[None])

        if return_trimesh:
            ret.append(mesh_tm)

        # preserve legacy behaviour when caller expected a single tensor
