...
class FlameTracker:
    ...
    def forward_flame(self, frame_idx, include_keyframes):
        ...
        neck_params = self.to_batch(self.neck_pose, indices)
        if frame_idx < 1412:
            neck_params = torch.zeros_like(neck_params)
    ...
    def compute_regularization_energy(self, result_dict, verts, verts_cano, lmks, albedos, frame_idx, include_keyframes, stage):
        ...
        if frame_idx > 1412:
            self.cfg.w.smooth_neck = 1.0
            self.cfg.w.smooth_rot = 3e3
            self.cfg.w.smooth_trans = 3e3
        else:
            self.cfg.w.smooth_neck = 3e1
            self.cfg.w.smooth_rot = 3e1
            self.cfg.w.smooth_trans = 3e2
