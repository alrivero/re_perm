...
def convert_variance_to_confidence(variance_map, var_min, var_max):
    ...
    normalized_variance = (clipped_variance - var_min) / (var_max - var_min)
...
def get_curvature_map(orientation_field, curve_length=65, mask=None,
                      confidence_map=None, mode='none', threshold=0.25, num_workers=-1):
    """
    Computes the curvature map in parallel.
    """
    ...
    print(f"  Dispatching {len(rows)} pixels to {num_workers if num_workers > 0 else os.cpu_count()} worker(s)...")
    ...
    print("  Assembling final curvature map...")
...
def main(args):
    ...
    # --- NEW: Auto-calculate variance range if not provided ---
    ...
    if args.save_average_map:
        os.makedirs(os.path.dirname(args.save_average_map), exist_ok=True)
    ...
    for map_name in tqdm(map_files, desc="Overall Progress"):
        ...
        if args.orient_format == 'angle':
            ...
            if orient_map_gray is None:
                # print(f"Warning: Could not read angle map {map_name}. Skipping.")
                continue
            orientation = orient_map_gray.astype(np.float32) * (np.pi / 180.0)
        else: # 'vector' format
            orient_map_rgb = cv2.imread(orient_map_path, cv.IMREAD_COLOR)
            if orient_map_rgb is None:
                # print(f"Warning: Could not read vector map {map_name}. Skipping.")
                continue
            ...
            dx_ch = channel_map[args.dx_channel]
            dy_ch = channel_map[args.dy_channel]
            dx = orient_map_rgb[:, :, dx_ch] * 2.0 - 1.0
            dy = orient_map_rgb[:, :, dy_ch] * 2.0 - 1.0
        ...
        if args.confidence_mode != 'none':
            ...
            if os.path.exists(conf_path):
                ...
                confidence_map = convert_variance_to_confidence(variance_map, args.var_min, args.var_max)
            else:
                # print(f"\nWarning: Confidence file not found for {basename}. Skipping confidence.")
                pass

        curvature = get_curvature_map(
            orientation, args.curve_length, 
            mask=mask, 
        ...
        if args.save_average_map:
            ...
            if total_curvature_sum.shape == curvature.shape:
                ...
                print(f"Warning: Shape mismatch. Skipping {basename} from average calculation.")
    ...
    if args.save_average_map and image_count > 0:
        print(f"\nCalculating and saving average map from {image_count} images...")
        average_curvature = total_curvature_sum / image_count
        np.save(args.save_average_map, average_curvature.astype(np.float16))
        print(f"Average map saved to {args.save_average_map}")
        avg_vis_path = os.path.splitext(args.save_average_map)[0] + '.png'
        avg_norm = cv2.normalize(average_curvature, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate curvature maps from orientation maps.')
    
    parser.add_argument('--orient_dir', required=True, type=str, help='Input directory for orientation maps.')
    parser.add_argument('--mask_dir', type=str, help="(Optional for 'vector' format) Input directory for external masks.")
    ...
    parser.add_argument('--curve_length', default=55, type=int, help='Pixel length of the curve used to calculate curvature.')
    parser.add_argument('--edge_margin', default=8, type=int, help='Pixels to ignore at the mask boundary to prevent artifacts.')
