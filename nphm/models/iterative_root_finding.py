...
def search(obs, cond, decoder_expr, anchors, multi_corresp=True):
    ...
    def _func(xc_opt, mask=None):
        if multi_corresp:
            ...
            out = decoder_expr({'queries': xc_opt.reshape(1, xc_opt.shape[0], 3), 'cond': cond, 'anchors': anchors}) #TODO: do I need mask here?
            xd_opt = out['offsets'] + xc_opt.reshape(1, xc_opt.shape[0], 3)
        else:
            #if cond.shape[0] != 1:
            #    out = decoder_expr({'queires': xc_opt.reshape(1, xc_opt.shape[0], 3),
            #                        'cond': cond.reshape(1, -1, cond.shape[2]),
            #                        'anchors': None if anchors is None else anchors.reshape(1, -1, anchors.shape[2], 3)})  # TODO: do I need mask here?
            #else:
            out = decoder_expr({'queries': xc_opt.reshape(1, xc_opt.shape[0], 3), 'cond': cond, 'anchors': anchors})  # TODO: do I need mask here?
            xd_opt = out['offsets'] + xc_opt.reshape(1, xc_opt.shape[0], 3)
        if obs.shape[0] != 1:
